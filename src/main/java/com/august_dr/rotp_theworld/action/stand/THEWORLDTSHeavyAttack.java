...
public class THEWORLDTSHeavyAttack extends StandEntityAction implements IHasStandPunch {
    ...
    protected void preTaskInit(World world, IStandPower standPower, StandEntity standEntity, ActionTarget target) {
        ...
        if (!world.isClientSide() || standEntity.isManuallyControlled()) {
            ...
            if (aimingEntity != null) {
                ...
                int timeStopTicks = blink.getMaxImpliedTicks(standPower);
                ...
                if (!world.isClientSide()) {
                    world.playSound(null, standEntity.getX(), standEntity.getY(), standEntity.getZ(),
                            InitSounds.THE_WORLD_TIME_STOP_BLINK.get(), SoundCategory.AMBIENT, 1.0F, 1.0F);
    ...
    public StandEntityPunch punchEntity(StandEntity stand, Entity target, StandEntityDamageSource dmgSource) {
        ...
        return new TheWorldTSHeavyPunch(stand, target, dmgSource)
                .setVoiceLineOnKill(InitSounds.DIEGO_THIS_IS_THE_WORLD)
    ...
    public StandBlockPunch punchBlock(StandEntity stand, BlockPos pos, BlockState state, Direction face) {
        return new StandEntityHeavyAttack.HeavyPunchBlockInstance(stand, pos, state, face)
                .impactSound(ModSounds.THE_WORLD_PUNCH_HEAVY);
    }
